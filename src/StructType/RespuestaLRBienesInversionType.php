...
class RespuestaLRBienesInversionType extends RespuestaComunAltaType
{
     ...
     * - maxOccurs: 10000
     * - minOccurs: 0
     * @var \ClonixDev\AeatSii\StructType\RespuestaBienType[]
     */
     ...
     * @param \ClonixDev\AeatSii\StructType\RespuestaBienType[] $respuestaLinea
     */
     ...
     * @return \ClonixDev\AeatSii\StructType\RespuestaBienType[]|null
     */
    ...
    public static function validateRespuestaLineaForArrayConstraintsFromSetRespuestaLinea(array $values = array())
    {
        $message = '';
        $invalidValues = [];
        foreach ($values as $respuestaLRBienesInversionTypeRespuestaLineaItem) {
            // validation for constraint: itemType
            if (!$respuestaLRBienesInversionTypeRespuestaLineaItem instanceof \ClonixDev\AeatSii\StructType\RespuestaBienType) {
                $invalidValues[] = is_object($respuestaLRBienesInversionTypeRespuestaLineaItem) ? get_class($respuestaLRBienesInversionTypeRespuestaLineaItem) : sprintf('%s(%s)', gettype($respuestaLRBienesInversionTypeRespuestaLineaItem), var_export($respuestaLRBienesInversionTypeRespuestaLineaItem, true));
            }
        }
        if (!empty($invalidValues)) {
            $message = sprintf('The RespuestaLinea property can only contain items of type \ClonixDev\AeatSii\StructType\RespuestaBienType, %s given', is_object($invalidValues) ? get_class($invalidValues) : (is_array($invalidValues) ? implode(', ', $invalidValues) : gettype($invalidValues)));
        }
        unset($invalidValues);
        return $message;
    }
     ...
     * @param \ClonixDev\AeatSii\StructType\RespuestaBienType[] $respuestaLinea
     * @return \ClonixDev\AeatSii\StructType\RespuestaLRBienesInversionType
     */
     ...
     * @throws \InvalidArgumentException
     * @param \ClonixDev\AeatSii\StructType\RespuestaBienType $item
     * @return \ClonixDev\AeatSii\StructType\RespuestaLRBienesInversionType
     */
    public function addToRespuestaLinea(\ClonixDev\AeatSii\StructType\RespuestaBienType $item)
    {
        // validation for constraint: itemType
        if (!$item instanceof \ClonixDev\AeatSii\StructType\RespuestaBienType) {
            throw new \InvalidArgumentException(sprintf('The RespuestaLinea property can only contain items of type \ClonixDev\AeatSii\StructType\RespuestaBienType, %s given', is_object($item) ? get_class($item) : (is_array($item) ? implode(', ', $item) : gettype($item))), __LINE__);
        }
        // validation for constraint: maxOccurs(10000)
