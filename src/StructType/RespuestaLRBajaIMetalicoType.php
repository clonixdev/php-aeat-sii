...
class RespuestaLRBajaIMetalicoType extends RespuestaComunBajaType
{
     ...
     * - maxOccurs: 10000
     * - minOccurs: 0
     * @var \ClonixDev\AeatSii\StructType\RespuestaMetalicoBajaType[]
     */
     ...
     * @param \ClonixDev\AeatSii\StructType\RespuestaMetalicoBajaType[] $respuestaLinea
     */
     ...
     * @return \ClonixDev\AeatSii\StructType\RespuestaMetalicoBajaType[]|null
     */
    ...
    public static function validateRespuestaLineaForArrayConstraintsFromSetRespuestaLinea(array $values = array())
    {
        $message = '';
        $invalidValues = [];
        foreach ($values as $respuestaLRBajaIMetalicoTypeRespuestaLineaItem) {
            // validation for constraint: itemType
            if (!$respuestaLRBajaIMetalicoTypeRespuestaLineaItem instanceof \ClonixDev\AeatSii\StructType\RespuestaMetalicoBajaType) {
                $invalidValues[] = is_object($respuestaLRBajaIMetalicoTypeRespuestaLineaItem) ? get_class($respuestaLRBajaIMetalicoTypeRespuestaLineaItem) : sprintf('%s(%s)', gettype($respuestaLRBajaIMetalicoTypeRespuestaLineaItem), var_export($respuestaLRBajaIMetalicoTypeRespuestaLineaItem, true));
            }
        }
        if (!empty($invalidValues)) {
            $message = sprintf('The RespuestaLinea property can only contain items of type \ClonixDev\AeatSii\StructType\RespuestaMetalicoBajaType, %s given', is_object($invalidValues) ? get_class($invalidValues) : (is_array($invalidValues) ? implode(', ', $invalidValues) : gettype($invalidValues)));
        }
        unset($invalidValues);
        return $message;
    }
     ...
     * @param \ClonixDev\AeatSii\StructType\RespuestaMetalicoBajaType[] $respuestaLinea
     * @return \ClonixDev\AeatSii\StructType\RespuestaLRBajaIMetalicoType
     */
     ...
     * @throws \InvalidArgumentException
     * @param \ClonixDev\AeatSii\StructType\RespuestaMetalicoBajaType $item
     * @return \ClonixDev\AeatSii\StructType\RespuestaLRBajaIMetalicoType
     */
    public function addToRespuestaLinea(\ClonixDev\AeatSii\StructType\RespuestaMetalicoBajaType $item)
    {
        // validation for constraint: itemType
        if (!$item instanceof \ClonixDev\AeatSii\StructType\RespuestaMetalicoBajaType) {
            throw new \InvalidArgumentException(sprintf('The RespuestaLinea property can only contain items of type \ClonixDev\AeatSii\StructType\RespuestaMetalicoBajaType, %s given', is_object($item) ? get_class($item) : (is_array($item) ? implode(', ', $item) : gettype($item))), __LINE__);
        }
        // validation for constraint: maxOccurs(10000)
