...
class RespuestaLRFEmitidasType extends RespuestaComunAltaType
{
     ...
     * - maxOccurs: 10000
     * - minOccurs: 0
     * @var \ClonixDev\AeatSii\StructType\RespuestaExpedidaType[]
     */
    public $RespuestaLinea;
    /**
     * Constructor method for RespuestaLRFEmitidasType
     * @uses RespuestaLRFEmitidasType::setRespuestaLinea()
     * @param \ClonixDev\AeatSii\StructType\RespuestaExpedidaType[] $respuestaLinea
     */
     ...
     * @return \ClonixDev\AeatSii\StructType\RespuestaExpedidaType[]|null
     */
    ...
    public static function validateRespuestaLineaForArrayConstraintsFromSetRespuestaLinea(array $values = array())
    {
        $message = '';
        $invalidValues = [];
        foreach ($values as $respuestaLRFEmitidasTypeRespuestaLineaItem) {
            // validation for constraint: itemType
            if (!$respuestaLRFEmitidasTypeRespuestaLineaItem instanceof \ClonixDev\AeatSii\StructType\RespuestaExpedidaType) {
                $invalidValues[] = is_object($respuestaLRFEmitidasTypeRespuestaLineaItem) ? get_class($respuestaLRFEmitidasTypeRespuestaLineaItem) : sprintf('%s(%s)', gettype($respuestaLRFEmitidasTypeRespuestaLineaItem), var_export($respuestaLRFEmitidasTypeRespuestaLineaItem, true));
            }
        }
        if (!empty($invalidValues)) {
            $message = sprintf('The RespuestaLinea property can only contain items of type \ClonixDev\AeatSii\StructType\RespuestaExpedidaType, %s given', is_object($invalidValues) ? get_class($invalidValues) : (is_array($invalidValues) ? implode(', ', $invalidValues) : gettype($invalidValues)));
        }
        unset($invalidValues);
        return $message;
    }
     ...
     * @throws \InvalidArgumentException
     * @param \ClonixDev\AeatSii\StructType\RespuestaExpedidaType $item
     * @return \ClonixDev\AeatSii\StructType\RespuestaLRFEmitidasType
     */
    public function addToRespuestaLinea(\ClonixDev\AeatSii\StructType\RespuestaExpedidaType $item)
    {
        // validation for constraint: itemType
        if (!$item instanceof \ClonixDev\AeatSii\StructType\RespuestaExpedidaType) {
            throw new \InvalidArgumentException(sprintf('The RespuestaLinea property can only contain items of type \ClonixDev\AeatSii\StructType\RespuestaExpedidaType, %s given', is_object($item) ? get_class($item) : (is_array($item) ? implode(', ', $item) : gettype($item))), __LINE__);
        }
        // validation for constraint: maxOccurs(10000)
