...
class RespuestaLROComunitariasType extends RespuestaComunAltaType
{
     ...
     * - maxOccurs: 10000
     * - minOccurs: 0
     * @var \ClonixDev\AeatSii\StructType\RespuestaComunitariaType[]
     */
     ...
     * @param \ClonixDev\AeatSii\StructType\RespuestaComunitariaType[] $respuestaLinea
     */
     ...
     * @return \ClonixDev\AeatSii\StructType\RespuestaComunitariaType[]|null
     */
    ...
    public static function validateRespuestaLineaForArrayConstraintsFromSetRespuestaLinea(array $values = array())
    {
        $message = '';
        $invalidValues = [];
        foreach ($values as $respuestaLROComunitariasTypeRespuestaLineaItem) {
            // validation for constraint: itemType
            if (!$respuestaLROComunitariasTypeRespuestaLineaItem instanceof \ClonixDev\AeatSii\StructType\RespuestaComunitariaType) {
                $invalidValues[] = is_object($respuestaLROComunitariasTypeRespuestaLineaItem) ? get_class($respuestaLROComunitariasTypeRespuestaLineaItem) : sprintf('%s(%s)', gettype($respuestaLROComunitariasTypeRespuestaLineaItem), var_export($respuestaLROComunitariasTypeRespuestaLineaItem, true));
            }
        }
        if (!empty($invalidValues)) {
            $message = sprintf('The RespuestaLinea property can only contain items of type \ClonixDev\AeatSii\StructType\RespuestaComunitariaType, %s given', is_object($invalidValues) ? get_class($invalidValues) : (is_array($invalidValues) ? implode(', ', $invalidValues) : gettype($invalidValues)));
        }
        unset($invalidValues);
        return $message;
    }
     ...
     * @param \ClonixDev\AeatSii\StructType\RespuestaComunitariaType[] $respuestaLinea
     * @return \ClonixDev\AeatSii\StructType\RespuestaLROComunitariasType
     */
     ...
     * @throws \InvalidArgumentException
     * @param \ClonixDev\AeatSii\StructType\RespuestaComunitariaType $item
     * @return \ClonixDev\AeatSii\StructType\RespuestaLROComunitariasType
     */
    public function addToRespuestaLinea(\ClonixDev\AeatSii\StructType\RespuestaComunitariaType $item)
    {
        // validation for constraint: itemType
        if (!$item instanceof \ClonixDev\AeatSii\StructType\RespuestaComunitariaType) {
            throw new \InvalidArgumentException(sprintf('The RespuestaLinea property can only contain items of type \ClonixDev\AeatSii\StructType\RespuestaComunitariaType, %s given', is_object($item) ? get_class($item) : (is_array($item) ? implode(', ', $item) : gettype($item))), __LINE__);
        }
        // validation for constraint: maxOccurs(10000)
